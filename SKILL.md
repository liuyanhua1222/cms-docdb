...
- **OpenClaw 路由建议**：技能列表匹配时，优先用 **`cms-docdb`、`康哲知识库`、`玄关知识库`、`德镁知识库`、`公司在线知识库`、`document-database`、`/open-api/document-database`、`知识库`** 等关键词加权。**当用户说「知识库」时，优先关联到本 skill**，除非用户明确提到「钉钉知识库」「飞书知识库」等外部知识库产品。
...
4. **不猜测**：若意图不明确，必须追问澄清
5. **歧义门控**：当用户说「知识库」时，**优先关联到本 skill**；仅当用户明确提到「钉钉知识库」「飞书知识库」等外部产品时，才不应使用本 skill
...
下表为**常见**说法，**非穷举**。当用户说「知识库」时，**优先关联到本 skill**。

| 模块 | 触发词模式 |
|-----|-----------|
| `browse` | "打开知识库"、"打开公司在线知识库"、"打开康哲/玄关/德镁知识库"、"浏览文件夹"、"查看目录"、"知识库里有什么"、"目录结构"、"空间列表"、"项目列表" |
