...
| 模块 | 触发词模式 |
|-----|-----------|
| `browse` | "知识库"、"打开知识库"、"打开公司在线知识库"、"打开康哲/玄关/德镁知识库"、"浏览文件夹"、"查看目录"、"知识库里有什么"、"目录结构"、"空间列表"、"项目列表"、"上传记录"、"最近上传了什么"、"最近使用"、"最近看过什么文件"、"最近下载过什么" |
...
| `share` | "分享文件给xxx"、"把这个文件分享给xxx"、"授权给xxx看"、"给xxx开权限"、"协同分享"、"分享协作"、"撤销分享"、"取消xxx的分享权限"、"分享给我的"、"我的分享" |
| `apply` | "申请权限"、"申请访问"、"我的申请"、"待我审批"、"审批申请"、"查询审批人"、"添加空间成员" |
| `grant` | "目录授权"、"空间目录授权"、"收回目录授权"、"撤销空间授权" |
...
| `parameter-extractor.py` | 参数提取和缺失提示生成（优化版：识别空间名+目录名候选词，标记需要智能匹配） |
| `project-matcher.py` | 智能空间名称匹配器（在实际空间列表中进行模糊匹配，避免分词错误） |
| `folder-navigator.py` | 智能目录导航器（递归搜索目录，支持多级路径导航和模糊匹配） |

### 触发流程图

```text
用户输入（如："保存到康哲知识库的产品资料目录"）
    ↓
...
参数提取 (parameter-extractor.py)
    → 识别空间名候选词：["康哲", "知识库"]
    → 识别目录名候选词：["产品资料"]
    → 标记：needs_project_list = true
    → 标记：needs_folder_navigation = true
    ↓
┌──────────────────────────────────────────────┐
│ 步骤1: 空间匹配（如果 needs_project_list）   │
└──────────────────────────────────────────────┘
    ↓
获取空间列表（根据意图选择接口）
    • upload → get-uploadable-list.py （可上传空间）
    • query/browse → get-project-list.py （可访问空间）
    ↓
智能匹配 (project-matcher.py)
    → 在实际空间列表中模糊匹配候选词
    ↓
┌─────────────────────────────────────┐
│ 空间匹配结果判断：                  │
...
└─────────────────────────────────────┘
    ↓ (得到 project_id)
┌──────────────────────────────────────────────┐
│ 步骤2: 目录导航（如果 needs_folder_navigation）│
└──────────────────────────────────────────────┘
    ↓
智能目录导航 (folder-navigator.py)
    → 在空间内递归搜索目录（最多3层）
    → 支持模糊匹配和路径导航
    ↓
┌─────────────────────────────────────┐
...
2. **目录匹配失败**："放到产品资料目录" → 盲目搜索 → "找不到目录"或"无权限" ❌
...
用户："保存到康哲知识库"
  ↓
提取候选词：["康哲", "知识库"] 或 ["康哲知识库"]
  ↓
调用 get-uploadable-list.py → 获取用户有上传权限的空间
  ↓
调用 project-matcher.py → 智能匹配
  → 精确匹配：候选词完全匹配项目名
  → 包含匹配：项目名包含候选词
  → 组合匹配：多个候选词组合后匹配
  → 模糊匹配：相似度 > 0.6
