...
- 文档示例：`python3 scripts/share/search-emp-by-name.py "张三"`
- 实际执行：`python3 <skill-dir>/scripts/share/search-emp-by-name.py "张三"`（将 `<skill-dir>` 换成 skill 根目录绝对路径）
...
python3 scripts/share/upsert-file-share-grants.py 2029019008342265857 --emp-id 10001 --confirm YES

# 3.1) 分享成功后一并输出短链
python3 scripts/share/upsert-file-share-grants.py 2029019008342265857 --emp-id 10001 --confirm YES --print-share-url --source "open_api"

# 4) 显式指定权限（逗号分隔）
python3 scripts/share/upsert-file-share-grants.py 2029019008342265857 --emp-id 10001 --permissions "read,preview,download" --confirm YES

# 4.1) 显式指定到期日（yyyyMMdd）；不传时默认会按长期有效处理（dueDate=20991231）
python3 scripts/share/upsert-file-share-grants.py 2029019008342265857 --emp-id 10001 --permissions "read,preview" --due-date 20991231 --confirm YES

# 5) 不发送钉钉通知（用户明确要求时才用）
python3 scripts/share/upsert-file-share-grants.py 2029019008342265857 --emp-id 10001 --no-notice --confirm YES
...
python3 scripts/share/revoke-file-share-grants.py 2029019008342265857 --emp-ids 10002,10003 --confirm YES
```
