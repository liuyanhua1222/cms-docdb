...
API_PATH = "/document-database/fileGrant/apply/myApplies"


def main():
    p = DocdbArgumentParser(hint="""list-my-applies.py list-my-applies 按业务参数调用（无必填时可传空 argv）。
示例: openapi_skill_exec skillCode=cms-docdb toolName=list-my-applies argv=["--page-index", "1"]；缺参补齐后用同一 toolName 重试，禁止改用标准 exec
""")
