...
def set_current_app_code(app_code, app_name=None, user_id="default"):
    """设置当前应用通道 appCode（多轮复用）"""
    context = load_context(user_id)
    context["current_app_code"] = app_code
    if app_name:
        context["current_app_name"] = app_name
    save_context(context, user_id)
    return context
...
def main():
    ...
    parser.add_argument("--app-code", type=str, help="应用编码（set_app_code 命令）")
    ...
    try:
        ...
        elif cmd == "set_app_code":
            if args.app_code:
                context = set_current_app_code(args.app_code, args.app_name, user_id)
                result["data"] = context
            else:
                result = {"resultCode": -1, "resultMsg": "参数不足：需要 --app-code [--app-name]"}
