...
使用方式：
  python3 -B <skill-dir>/scripts/upload/create-folder.py --parent-id 0 --name "新产品" --project-id 10001 --confirm YES
...
def main():
    parser = DocdbArgumentParser(description="在指定父目录下创建文件夹", hint="""create-folder.py 必须提供 --parent-id 与 --name。
空间根 --parent-id 0 时必须 --project-id（禁止仅传 0 当作个人库捷径）。
真实写入还需 --confirm YES（可先 --dry-run）。
示例: python3 -B <skill-dir>/scripts/upload/create-folder.py --parent-id 0 --name "新产品" --project-id 10001 --confirm YES；缺参补齐后用同一 python 命令重试
""")
    parser.add_argument("--parent-id", type=int, required=True, help="父目录 fileId；空间根传 0 时必须同时传 --project-id")
    parser.add_argument("--name", type=str, required=True, help="文件夹名称（勿含 / 或 \）")
